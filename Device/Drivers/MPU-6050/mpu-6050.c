...
static void     MPU6050_vSetSampleRate(void);
static uint32_t MPU6050_ulObtainFactoryTrim(MPU6050_Sensor_t eSensor,
                                            Axis_t           eAxis);
static int32_t  MPU6050_lCalculateAccelFactoryTrim(const uint32_t ulFt,
                                                   const Axis_t   eAxis);
static int32_t  MPU6050_lCalculateGyroFactoryTrim(const uint32_t ulFt,
                                                  const Axis_t   eAxis);
static uint8_t  MPU6050_ucReadReg(uint8_t ucReg);
static void     MPU6050_vWriteReg(uint8_t ucReg, uint8_t ucVal);
...
void MPU6050_vInit(void)
{
    ...
    /* Bus test */
    ucId = MPU6050_ucReadReg(MPU6050_WHO_AM_I);
    if (ucId == 0xFF)
    {
    ...
    MPU6050_vWriteReg(MPU6050_PWR_MGMT_1, MPU6050_PWR_MGMT_1_DEVICE_RESET);
    MPU6050_vWriteReg(MPU6050_USER_CTRL, MPU6050_USER_CTRL_SIG_COND_RESET);

    /* Free to configure the device now */
    MPU6050_vSetSampleRate();
    MPU6050_vWriteReg(MPU6050_PWR_MGMT_1, MPU6050_PWR_MGMT_1_CLKSEL_X_GYRO);

    /* Perform self-testing */
    bRet  = MPU6050_bSelfTest(SENSOR_ACCEL);
    bRet &= MPU6050_bSelfTest(SENSOR_GYRO);
    if (bRet != true)
    {
        Journal_vWriteError(MPU6050_ERROR);
    }
...
static void MPU6050_vSetSampleRate(void)
{
    ...
    MPU6050_vWriteReg(MPU6050_SMPRT_DIV, MPU6050_SMPRT_DIV_SMPRT_DIV(1));
    MPU6050_vWriteReg(MPU6050_CONFIG, MPU6050_CONFIG_DLPF_CFG(1));
}
                                            ...
                                            Axis_t           eAxis)
{
    ...
    ulAxisTest  = MPU6050_ucReadReg(ulTestAxisTbl[eAxis]);
