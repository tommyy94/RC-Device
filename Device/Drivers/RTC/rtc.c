...
void RTC_vInit(void)
{
    ...
    RTC->RTC_IDR = RTC_IDR_TDERRDIS_Msk | RTC_IDR_CALDIS_Msk
                 | RTC_IDR_TIMDIS_Msk   | RTC_IDR_ALRDIS_Msk
                 | RTC_IDR_ACKDIS_Msk;
...
void RTC_Handler(void)
{
    ...
    {
        xTaskNotifyFromISR(xRtcTask, RTC_SEC_PERIOD, eSetBits, &xTaskWoken);
    }
...
void RTC_vTask(void *pvArg)
{
    ...
    while (1)
    {
        ulEvent = ulTaskNotifyTake(pdFALSE, portMAX_DELAY);
        ...
        {
            xRet = xQueueReceive(xTsQ, &xCal, pdMS_TO_TICKS(10));
            assert(xRet != errQUEUE_EMPTY);
        ...
        {
            xRet = xQueueSend(xTsQ, &xCal, pdMS_TO_TICKS(10));
            assert(xRet != errQUEUE_FULL);
        }
    }
}
